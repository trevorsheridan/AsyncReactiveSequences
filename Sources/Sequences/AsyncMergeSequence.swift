//
//  AsyncCombineLatestSequence 2.swift
//  AsyncReactiveSequences
//
//  Created by Trevor Sheridan on 6/29/25.
//

import Synchronization

public final class AsyncMergeSequence<each Upstream, Element>: AsyncSequence, Sendable
where Element: Sendable, repeat each Upstream: AsyncSequence & Sendable, repeat (each Upstream).Element == Element {
...
extension AsyncSequence {
    public func merge<Upstream>(isolation actor: isolated (any Actor)? = #isolation, _ upstream: Upstream) -> AsyncCombineLatestSequence<Self, Upstream>
    ...
    public func merge<Upstream, each Other>(isolation actor: isolated (any Actor)? = #isolation, _ upstream: Upstream, _ other: repeat each Other) -> AsyncCombineLatestSequence<Self, Upstream, repeat each Other>
